    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if segSize is None:
            ...
            if self.deep_sup_scale is not None: # use deep supervision technique
                ...
                # dim2 = int(feed_dict['img_data'].shape[2] / 20)
                # dim3 = int(feed_dict['img_data'].shape[3] / 20)
                # rem2 = dim2 % 8
                # rem3 = dim3 % 8
                # dim2 -= rem2
                # dim3 -= rem3
                # rdim2 = int(feed_dict['img_data'].shape[2] / dim2)
                # rdim3 = int(feed_dict['img_data'].shape[3] / dim3)
                # rvdim2 = random.randint(0, rdim2-1)
                # rvdim3 = random.randint(0, rdim3-1) 
                # rvcdim2 = int(dim2 / 8)
                # rvcdim3 = int(dim3 / 8)
                # pdim2 =  rvdim2 * dim2
                # pdim3 = rvdim3 * dim3
                # pdim2e = pdim2 + dim2
                # pdim3e = pdim3 + dim3
                # om = np.ones((feed_dict['img_data'].shape[0], feed_dict['img_data'].shape[1], feed_dict['img_data'].shape[2],feed_dict['img_data'].shape[3]))
                # om[:,:,pdim2:pdim2e, pdim3:pdim3e] = 0
                # om = torch.from_numpy(om).float().cuda().detach()
                # en_out2 = self.encoder(feed_dict['img_data'].cuda() * om, return_feature_maps=True)
            else:
                en_out = self.encoder(feed_dict['img_data'].cuda(), return_feature_maps=True)
                en_out1 = self.encoder1(feed_dict['img_data'].cuda(), return_feature_maps=True)
            ...
            CE_loss = self.crit(pred, feed_dict['seg_label'].cuda())
            contextual_loss1 = self.crit1(en_out[-1], en_out1[0])
            # contextual_loss2 = self.crit1(en_out2[-1][:,:,rvdim2*rvcdim2: (rvdim2+1)*rvcdim2,rvdim3*rvcdim3: (rvdim3+1)*rvcdim3], en_out[-1][:,:,rvdim2*rvcdim2: (rvdim2+1)*rvcdim2,rvdim3*rvcdim3: (rvdim3+1)*rvcdim3])
            loss = CE_loss + contextual_loss1 #+ contextual_loss2 
            
            # print(CE_loss, contextual_loss1, contextual_loss2)
...
class ResnetDilatedContext(nn.Module):
    def __init__(self, orig_resnet, dilate_scale=8):
        ...
        self.relu3 = orig_resnet.relu3
        self.maxpool = orig_resnet.maxpool
        # self.context_layer1 = orig_resnet.context_layer1
        # self.context_layer2 = orig_resnet.context_layer2
        self.layer1 = orig_resnet.layer1
        self.layer2 = orig_resnet.layer2
        self.layer3 = orig_resnet.layer3
        self.layer4 = orig_resnet.layer4
        ...
        self.self_attn_model = MultiHeadSelfAttention(dim=512,heads=1)
        self.self_attn_model1 = MultiHeadSelfAttention(dim=256,heads=1)        
    ...
    def forward(self, x, return_feature_maps=False):
        ...
        x_ = self.self_attn_model1(torch.reshape(x, (x.shape[0], x.shape[1],x.shape[2]* x.shape[3])).permute(0,2,1))
        x = torch.reshape(x_.permute(0,2,1), (x.shape[0], x.shape[1],x.shape[2], x.shape[3]))
        conv_out.append(x)
        ...
        # print(x.shape)
        # input()
        # mask = torch.zeros(x.shape[1]*x.shape[2], x.shape[1]*x.shape[2])  # tokens X tokens
        # mask[5:8, 5:8] = 1
        
        x_ = self.self_attn_model(torch.reshape(x, (x.shape[0], x.shape[1],x.shape[2]* x.shape[3])).permute(0,2,1))
        x = torch.reshape(x_.permute(0,2,1), (x.shape[0], x.shape[1],x.shape[2], x.shape[3]))
        conv_out.append(x)
