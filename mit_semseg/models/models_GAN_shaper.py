...
class SegmentationModule(SegmentationModuleBase):
    def __init__(self, net_enc, net_dec, net_disc, crit, deep_sup_scale=None):
    ...
    def forward(self, feed_dict, *, segSize=None):
        ...
        if segSize is None:
            ...
            if r == 0:
                disc_out = self.disc(torch.unsqueeze(pred[:,1,:,:],1).float())                
                GAN_loss = (1 - disc_out) * (1 - disc_out)
            else:
                disc_out = self.disc(torch.unsqueeze(feed_dict['seg_label'].cuda(),1).float())
                GAN_loss = (disc_out - 1) * (disc_out - 1)
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        x = self.disc(x)
        x = self.disc_classifier(x.view(x.shape[0], -1))
        return x
...
class ModelBuilder:
    ...
    @staticmethod
    def build_disc(weights=''):
        print("building disc")
        disc = Discriminator()
        
        disc.apply(ModelBuilder.weights_init)
        if len(weights) > 0:
            print('Loading weights for disc')
